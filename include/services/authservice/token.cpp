...
namespace token_service {
    authservice::Tokens *generate_tokens(authservice::UserDTO *user_dto) {
        ...
        authservice::AccessToken *access_token = jwtokens::create_access_token(payload, 30min);
        authservice::RefreshToken *refresh_token = jwtokens::create_refresh_token(payload, 43200min);
    ...
    grpc::Status delete_refresh_token(const authservice::RefreshToken token) {
        for (auto const &[key, val] : refresh_tokens) {
